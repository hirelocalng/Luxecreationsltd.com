...
function DivisionGallery() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/division-media?division=confectioneries&published=true`)
      .then(r => r.json())
      .then(res => {
        console.log('[Confectioneries Gallery] raw response:', res);
        console.log('[Confectioneries Gallery] item count:', res.data?.length ?? 'res.data missing — check res.error');
